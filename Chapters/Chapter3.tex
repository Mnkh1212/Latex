...
\node[data] (d) at (-3.2, 1) {%
    ...
    \scriptsize \textbf{MinIO} S3-нийцтэй объект хадгалалт (хуулга, аватар)
};
...
\subsection{Үндсэн загвар ба харилцааны (Class) диаграмм}
Сервер талын \texttt{internal/models} пакет дахь үндсэн ангийн (struct) бүтэц, тэдгээрийн талбар, метод болон хоорондын харилцааг доорх ангийн диаграммаар үзүүлэв. Энэ нь ER загвартай ойролцоо боловч программ хангамжийн түвшинд буюу GORM-ийн загварын давхаргыг тусгасан.
...
\caption{Class Diagram (Класс диаграмм) --- сервер талын загварууд ба харилцаа (композици: $\blacklozenge\!-$, ассоциаци: $\rightarrow$)}
\label{fig:class}
\end{figure}
...
\subsection{Хүснэгтүүдийн уялдаа (ER диаграмм)}
...
\caption{Entity-Relationship Diagram (ER диаграмм) --- 11 хүснэгтийн foreign-key уялдаа}
\label{fig:erd}
\end{figure}
...
\begin{itemize}
    ...
    \item \textbf{Нээлттэй эх + self-host.} AGPL лицензтэй, локал серверт суулгах боломжтой. Хувийн мэдээллийг өөрийн дэд бүтцэд хадгалах боломжтой нь Монголын банкны хуулгын мэдрэмтгий байдалд тохиромжтой.
    \item \textbf{Өндөр гүйцэтгэл.} Go хэлээр бичигдсэн, нэг node 100,000+ объект/секундийн уншилтыг даадаг.
    \item \textbf{Дотор суулгасан шифрлэлт ба хувилбарлалт (versioning).} \texttt{SSE-S3} буюу \texttt{SSE-KMS}-ээр сервер тал шифрлэх боломжтой, мөн хувилбарлалт идэвхжүүлбэл санамсаргүй устгалаас сэргийлдэг.
    \item \textbf{Контейнерт амархан суулгана.} \texttt{docker-compose.yml}-д нэг үйлчилгээ нэмэхэд хангалттай: \texttt{minio/minio:latest} дүрс, port 9000 (S3 API) ба port 9001 (Web Console).
\end{itemize}
...
\begin{enumerate}
    ...
    \item Боловсруулагч (\texttt{ai\_analysis.go} эсвэл \texttt{auth.go:UploadAvatar}) файлыг түр зуурын буферээс уншиж \texttt{minio-go} SDK-ийн \texttt{PutObject} аргаар bucket-д байршуулна.
    \item Үр дүнд буцсан \textbf{объектын түлхүүр} (\texttt{key}, жишээ нь \texttt{statement\_42\_1717000000.pdf}) нь \texttt{ai\_analyses.filename} эсвэл \texttt{users.avatar} талбарт хадгалагдана. \emph{Бодит файл DB-д ордоггүй}, зөвхөн заагч.
...
\subsection{MinIO ба локал \texttt{./uploads} хавтасны харьцуулалт}
\begin{table}[ht]
\centering
\footnotesize
...
Шифрлэлт            & Гар тохируулга                          & SSE-S3 дотор суулгасан \\ \hline
Хувилбарлалт         & Байхгүй                                 & Хувилбарлалт идэвхжүүлбэл санамсаргүй устгал сэргээгдэнэ \\ \hline
Гадаад сервис рүү шилжих & Кодыг шинээр бичих ёстой               & S3-нийцтэй тул AWS S3, B2, R2 рүү яг ижил кодоор \\ \hline
Нөөцлөлт, хуулбарлалт & Локал дисктэй уядаг                    & Хэд хэдэн node-д хуулбар буюу erasure code \\ \hline
...
\begin{itemize}
    ...
    \item Хариулт нь стандарт JSON формат, HTTP төлөвийн код зөв ашигласан (200, 201, 400, 401, 404, 409, 500),
...
Системийн хамгийн нарийн хэсэг нь банкны хуулгыг автомат байдлаар бүтэцтэй формат руу хөрвүүлж AI-д тэжээх хоолой юм.
...
\subsection{AI шинжилгээний sequence diagram}
Дээрх 7 алхмыг \texttt{ai\_analysis.go:AnalyzeStatement} боловсруулагчийн дотоод дуудлагын дарааллаар нь sequence diagram хэлбэрээр харуулъя. Энэ нь MinIO, Python parser, AI / LLM, PostgreSQL гэсэн 4 өөр үйлчилгээний хооронд хэрэглэгчийн ганц POST хүсэлтэд хариулахаар хийгдэх 11 дотоод дуудлагын зохион байгуулалтыг тодорхой үзүүлнэ.

\begin{figure}[ht]
\centering
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
    actor/.style={draw=darknavy, fill=accentteal!30, thick, rectangle, rounded corners, minimum width=2.2cm, minimum height=0.7cm, font=\footnotesize},
    msg/.style={->, thick, draw=darknavy, font=\scriptsize},
    ret/.style={->, thick, draw=accentgreen!60!black, dashed, font=\scriptsize},
    lbl/.style={font=\scriptsize, fill=white, inner xsep=4pt, inner ysep=1pt, sloped=false},
    note/.style={rectangle, rounded corners=3pt, draw=accentorange!85!black, fill=accentyellow!35, font=\scriptsize, align=center, very thick, inner sep=5pt}
]

% --- Жижигхэн зайтай байрлуулсан 6 lifeline (өргөн 18) ---
\node[actor] (u)  at (0, 0)    {Хэрэглэгч};
\node[actor] (be) at (3.6, 0)  {Сервер тал};
\node[actor] (mn) at (7.2, 0)  {MinIO};
\node[actor] (ps) at (10.8, 0) {Parser};
\node[actor] (ai) at (14.4, 0) {AI / LLM};
\node[actor] (db) at (18.0, 0) {DB};

\foreach \x in {0, 3.6, 7.2, 10.8, 14.4, 18.0} {
    \draw[dashed, very thin, gray!60] (\x, -0.4) -- (\x, -10.3);
}

% --- Шошго бүгд цагаан background-той тул шугам дээр давхрахгүй ---
\draw[msg] (0,    -1.0)  -- (3.6, -1.0)
    node[lbl, midway, above] {POST /ai/analysis (file + JWT)};

\draw[msg] (3.6,  -1.9)  -- (7.2, -1.9)
    node[lbl, midway, above] {PutObject (хуулга PDF/XLSX)};

\draw[ret] (7.2,  -2.7)  -- (3.6, -2.7)
    node[lbl, midway, above] {object key};

\draw[msg] (3.6,  -3.6)  -- (10.8, -3.6)
    node[lbl, midway, above] {POST /parse (multipart proxy)};

\draw[ret] (10.8, -4.5)  -- (3.6, -4.5)
    node[lbl, midway, above] {ParsedStatement JSON};

\draw[msg] (3.6,  -5.4)  -- (14.4, -5.4)
    node[lbl, midway, above] {system prompt + JSON контекст};

\draw[ret] (14.4, -6.3)  -- (3.6, -6.3)
    node[lbl, midway, above] {summary + 3--5 зөвлөмж};

\draw[msg] (3.6,  -7.2)  -- (18.0, -7.2)
    node[lbl, midway, above] {INSERT ai\_analyses (JSON багана)};

\draw[msg] (3.6,  -8.1)  -- (18.0, -8.1)
    node[lbl, midway, above] {BatchCreate transactions};

\draw[msg] (3.6,  -9.0)  -- (18.0, -9.0)
    node[lbl, midway, above] {INSERT activity\_log};

\draw[ret] (3.6,  -9.9)  -- (0, -9.9)
    node[lbl, midway, above] {AIAnalysisResponse JSON};

% --- Нот хайрцаг AI lifeline дээр (5.4 ба 6.3 хоорондох мессежийн ойролцоо) ---
\node[note, anchor=west, font=\tiny, text width=4.5cm]
    at (14.7, -5.85)
    {\textbf{AI давхрагын fallback:}\\
     Gemini амжилтгүй (geo-block / 429 / 5xx) $\Rightarrow$
     OpenRouter руу автомат нөөц шилжилт};

\end{tikzpicture}
}
\caption{Sequence Diagram (Дарааллын диаграмм) --- Банкны хуулгын AI шинжилгээ}
\label{fig:seq-analysis}
\end{figure}

\textbf{Тайлбар.} Хар цул шугам нь хүсэлт (request), тасархай ногоон шугам нь хариулт (response). Шар хайрцагт AI давхаргын дотоод нөөц шилжилтийн зарчмыг тусгайлан тэмдэглэв --- хэрэглэгч талд энэ үйл явц харагдахгүй, зөвхөн сервер тал дотор шийдэгдэнэ. Хэрэглэгч талаас гарсан \emph{нэг} POST хүсэлт нь сервер тал дотор \textbf{4 өөр үйлчилгээ} (MinIO, Parser, AI, DB)-тэй харьцаж 11 дотоод дуудлага хийгдээд эцэст нь \emph{нэг} JSON хариулт болж буцна.
...
\end{tikzpicture}
\caption{Sequence Diagram (Дарааллын диаграмм) --- AI чат}
\label{fig:seq-chat}
\end{figure}

\subsection{Гүйлгээ нэмэх үйл ажиллагааны диаграмм}
Дараах үйл ажиллагааны диаграмм нь хэрэглэгч шинэ гүйлгээ нэмэхэд системд хийгдэх алхмууд, шийдвэрийн салаалт, хэвийн ба алдаатай гарцыг тодорхой үзүүлж байна.
...
\end{tikzpicture}
\caption{Activity Diagram (Үйл ажиллагааны диаграмм) --- гүйлгээ нэмэх}
...
\begin{itemize}
    ...
    \item \textbf{Гар утас төвтэй (mobile-first) хандлага.} Гар утасны дэлгэцэд зориулсан, илүүдэл том элементгүй.
...
Доорх диаграммаас гол дэлгэцүүдийн схем зохион байгуулалтыг харж болно.
...
\subsection{Нөөц шилжилтийн нөхцөлүүд}
...
\textbf{Дүгнэлт.} Энэ бүлэгт системийн архитектур, сервер талын кодын зохион байгуулалт, загваруудын class диаграмм, өгөгдлийн сангийн ER загвар, MinIO объект хадгалалтын дизайн, REST API цэгүүд, AI шинжилгээний долоон алхамт урсгал, хоёр дарааллын диаграмм (AI чат болон банкны хуулгын шинжилгээ), гүйлгээ нэмэх үйл ажиллагааны диаграмм, хэрэглэгч талын дэлгэц, аюулгүй байдлын механизм болон хэрэгжүүлэлтийн тоон үзүүлэлтийг танилцууллаа. Дараагийн хэсэгт дипломын ажлын нэгдсэн дүгнэлт, цаашдын хөгжүүлэлтийн чиглэлийг авч үзнэ.
