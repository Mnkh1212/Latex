...
\begin{tikzpicture}[
    ...
    note/.style={rectangle, rounded corners=3pt, draw=accentorange!85!black, fill=accentyellow!35, font=\scriptsize, align=center, very thick, inner sep=4pt}
]

\node[actor] (u)  at (0, 0)   {Хэрэглэгч};
\node[actor] (be) at (3, 0)   {Сервер тал};
\node[actor] (mn) at (6, 0)   {MinIO};
\node[actor] (ps) at (9, 0)   {Parser};
\node[actor] (ai) at (12, 0)  {AI / LLM};
\node[actor] (db) at (15, 0)  {DB};

\foreach \x in {0,3,6,9,12,15} {
    \draw[dashed, thick, gray] (\x, -0.4) -- (\x, -9);
}

\draw[msg] (0, -1.0)  -- node[above]{POST /ai/analysis (file + JWT)} (3, -1.0);
\draw[msg] (3, -1.7)  -- node[above]{PutObject (хуулга PDF/XLSX)} (6, -1.7);
\draw[ret] (6, -2.4)  -- node[above]{object key} (3, -2.4);
\draw[msg] (3, -3.1)  -- node[above]{POST /parse (multipart proxy)} (9, -3.1);
\draw[ret] (9, -3.8)  -- node[above]{ParsedStatement JSON} (3, -3.8);
\draw[msg] (3, -4.5)  -- node[above]{системийн зааварчилгаа + JSON контекст} (12, -4.5);
\draw[ret] (12, -5.2) -- node[above]{summary + 3--5 зөвлөмж} (3, -5.2);
\draw[msg] (3, -5.9)  -- node[above]{INSERT ai\_analyses (JSON багана)} (15, -5.9);
\draw[msg] (3, -6.6)  -- node[above]{BatchCreate transactions} (15, -6.6);
\draw[msg] (3, -7.3)  -- node[above]{INSERT activity\_log} (15, -7.3);
\draw[ret] (3, -8.0)  -- node[above]{AIAnalysisResponse JSON} (0, -8.0);

\node[note] at (12, -8.7) {AI давхарга: Gemini (анхдагч) амжилтгүй (geo-block / 429 / 5xx) \\ $\Rightarrow$ OpenRouter руу автомат нөөц шилжилт};
